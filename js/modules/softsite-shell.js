...
(function () {
  ...
  const NAV = [
    ...
    ['FIX & EXPORT', [
      ['boq', '\u{1F9EE}', 'Quantity Take-off'], ['propertyeditor', '✏', 'Property Editor'],
      ['export', '\u{1F4E4}', 'Export Reports'], ['cobie', '\u{1F4CB}', 'COBie Export']]],
    ['TOOLS', [
      ['ids', '✓', 'IDS Checker'], ['clash', '\u{1F4A5}', 'Clash Detection'], ['bcf', '\u{1F501}', 'BCF Issues'], ['security', '\u{1F6E1}', 'VERIFIQ Security'], ['merge', '⊕', 'IFC Merge'],
  ...
  function build() {
    ...
    shell.querySelectorAll('.vq-navbtn').forEach(function (b) {
      b.addEventListener('click', function () {
        if (b.dataset.page === 'boq') { if (window.SoftSiteBoq) SoftSiteBoq.show(); }
        else if (b.dataset.page === 'ids') { if (window.SoftSiteIds) SoftSiteIds.show(); }
        else if (b.dataset.page === 'clash') { if (window.SoftSiteClash) SoftSiteClash.show(); }
        else if (b.dataset.page === 'bcf') { if (window.SoftSiteBcf) SoftSiteBcf.show(); }
        else if (b.dataset.page === 'security') { if (window.VerifiqSecurity) VerifiqSecurity.show(); }
        else if (window.App && App.navigate) App.navigate(b.dataset.page);
