...
const VEngine = (() => {
  ...
  // ── VERIFIQ Key Vault: seal any connected API/AI key at rest, so localStorage holds only
  //    AES ciphertext. The encryption secret is compiled into the WASM engine (C#/VqCrypto),
  //    so JS cannot read it. On the desktop (WebView2, no DotNet bridge) this transparently
  //    falls back to plaintext - localStorage there is the app's own isolated store.
  function _salt() {
    let s = localStorage.getItem('vq_salt');
    if (!s) {
      try { const a = new Uint8Array(16); crypto.getRandomValues(a); s = btoa(String.fromCharCode.apply(null, a)); }
      catch (e) { s = 'vq-static-salt'; }
      localStorage.setItem('vq_salt', s);
    }
    return s;
  }
  function _vault() { return typeof DotNet !== 'undefined' && !!DotNet.invokeMethod; }
  function _seal(plain) {
    if (!plain) return '';
    try { return _vault() ? DotNet.invokeMethod('VERIFIQ.SoftSite', 'VqSeal', plain, _salt()) : plain; }
    catch (e) { return plain; }
  }
  function _unseal(blob) {
    if (!blob) return null;
    try { return _vault() ? DotNet.invokeMethod('VERIFIQ.SoftSite', 'VqUnseal', blob, _salt()) : blob; }
    catch (e) { return String(blob).indexOf('vqk1:') === 0 ? null : blob; }
  }
  ...
  function getKey(id) {
    ...
    const v = localStorage.getItem(e.lsKey);
    return v ? _unseal(v) : null;
  }

  function setKey(id, val) {
    const e = ENGINES[id];
    if (!e || !e.lsKey) return;
    if (val && val.trim()) localStorage.setItem(e.lsKey, _seal(val.trim()));
  ...
  async function _callCustom(cx, prompt, sys) {
    ...
    const k = _unseal(cx.key);
    if (style === 'x-api-key' && k) {
      headers['x-api-key'] = k;
      headers['anthropic-version'] = '2023-06-01';
    } else if (style === 'basic' && k) {
      headers['Authorization'] = 'Basic ' + btoa(k);
    } else if (style !== 'none' && k) {
