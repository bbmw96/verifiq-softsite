...
<div style="height:100%;display:flex;flex-direction:column;overflow:hidden">
  ...
  <div style="display:flex;align-items:center;gap:6px;padding:7px 16px;background:#06111f;border-bottom:1px solid #162843;flex-shrink:0;flex-wrap:wrap">
    ...
    <select id="view-f" onchange="ResultsPage.loadView(this.value)" title="Smart Views - saved filter presets" style="height:26px;padding:0 6px;font-size:11px;border:1px solid var(--border);border-radius:5px;background:#0a1628;color:var(--mid-grey);cursor:pointer">${_viewOpts()}</select>
    <button onclick="ResultsPage.saveView()" title="Save the current filters as a Smart View" style="height:26px;padding:0 8px;font-size:11px;border:1px solid var(--teal);border-radius:5px;background:transparent;color:var(--teal);cursor:pointer;white-space:nowrap">💾 Save view</button>
  </div>
  ...
  function _rows(findings) {
    ...
    return findings.map((f, i) => {
      ...
      const na  = _naSet.has(naKey(f));
      return `<tr style="background:${s.bg};border-bottom:1px solid #0f1e30;transition:background .15s;cursor:pointer${na?';opacity:.45':''}" onclick="ResultsPage.rowClick(event,${i})" title="Click for full detail and actual property values" onmouseenter="this.style.filter='brightness(1.15)'" onmouseleave="this.style.filter=''">
        ...
        <td style="padding:8px 10px;vertical-align:top">
          <div style="display:flex;flex-direction:column;gap:4px">
            ...
            <button style="font-size:10px;padding:2px 6px;border:1px solid #1d3354;border-radius:4px;background:transparent;color:${na?'var(--teal)':'#5b7fa6'};cursor:pointer;white-space:nowrap"
              onclick="event.stopPropagation();ResultsPage.toggleNA(${i})">${na?'↩ Undo':'🚫 N/A'}</button>
          </div>
        </td>
      </tr>`;
    }).join('');
  ...
  // "Not Applicable" - dismiss findings that do not apply to this submission. Persisted
  // per element+rule (localStorage) so the state survives re-validation and page reloads.
  var _naSet = (function () {
    try { return new Set(JSON.parse(localStorage.getItem('vq_na') || '[]')); } catch (e) { return new Set(); }
  })();
  function naKey(f) { return (f.guid || '') + '|' + (f.ruleRef || f.check || ''); }
  function toggleNA(idx) {
    var f = _shownFindings[idx];
    if (!f) return;
    var k = naKey(f);
    if (_naSet.has(k)) _naSet.delete(k); else _naSet.add(k);
    try { localStorage.setItem('vq_na', JSON.stringify(Array.from(_naSet))); } catch (e) { /* storage disabled */ }
    af();
  }

  // Smart Views - save the current filter combination as a named preset, so a QP can jump
  // straight back to "SCDF criticals" or "Level 3 doors" without re-picking every filter.
  var _views = (function () {
    try { return JSON.parse(localStorage.getItem('vq_views') || '{}'); } catch (e) { return {}; }
  })();
  function _saveViews() { try { localStorage.setItem('vq_views', JSON.stringify(_views)); } catch (e) { /* storage disabled */ } }
  function _viewOpts() {
    var o = '<option value="">\u{1F4CC} Views…</option>';
    Object.keys(_views).forEach(function (n) { o += '<option value="' + VUtils.esc(n) + '">' + VUtils.esc(n) + '</option>'; });
    return o;
  }
  function saveView() {
    var n = (window.prompt('Name this Smart View (e.g. "SCDF criticals"):') || '').trim();
    if (!n) return;
    _views[n] = { sev:_filters.sev, disc:_filters.disc, ifc:_filters.ifc, agency:_filters.agency,
                  storey:_filters.storey, gw:_filters.gw, check:_filters.check, search:_filters.search };
    _saveViews();
    af();
  }
  function loadView(n) {
    if (!n || !_views[n]) return;
    var v = _views[n];
    var map = { sev:'sev-f', disc:'disc-f', ifc:'ifc-f', agency:'agency-f', storey:'storey-f', gw:'gw-f', check:'check-f', search:'srch-f' };
    Object.keys(map).forEach(function (k) {
      var e = document.getElementById(map[k]);
      if (e) e.value = v[k] || '';
    });
    _filters.search = v.search || '';   // af() reads the selects; search is applied by as()
    af();
  }
