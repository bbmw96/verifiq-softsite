...
const AiAssistantPage = (() => {
  ...
  function render() {
    ...
    return `<div style="display:flex;flex-direction:column;height:100%;min-height:0;background:#080f1d;overflow:hidden">
  ...
  function showSettings() {
    ...
    const hint = _el('div','font-size:11px;color:#5b7fa6;margin-bottom:16px','API keys are AES-encrypted at rest in the VERIFIQ Key Vault (the secret is compiled into the WASM engine, so no page script can read it) - localStorage only ever holds ciphertext. Keys are sent only to the provider you chose, direct from your browser, never logged and never through a VERIFIQ server. Add any OpenAI-compatible API, corporate AI gateway, or proprietary AI service.');
