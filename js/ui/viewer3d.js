...
const Viewer3DPage = (() => {
  ...
  function render() {
    ...
    if(!files.length) return `<div style="display:flex;align-items:center;justify-content:center;height:100%;flex-direction:column;gap:12px;color:#6b8faf">
      ...
      <button onclick="VBridge.openFile()" style="padding:10px 24px;background:var(--teal);color:#000;border:none;border-radius:8px;font-weight:700;cursor:pointer;font-size:13px">Open IFC File</button>
    </div>`;
    ...
    return `<div id="v-wrap" style="display:flex;flex-direction:column;height:100%;min-height:0;background:#0d1117;overflow:hidden">
  ...
  <div style="display:flex;align-items:center;flex-wrap:wrap;gap:3px;padding:5px 8px;background:#071220;border-bottom:1px solid #0f1e30;flex-shrink:0">
    ...
    <span style="width:1px;height:20px;background:#1a3354;margin:0 3px"></span>
    <select id="v-export" onchange="Viewer3DPage.exportModel(this.value);this.selectedIndex=0"
            title="Export the loaded model for AutoCAD, ArchiCAD, Revit, SketchUp or Blender" style="${S}">
      <option value="">📤 Export model…</option>
      <option value="obj">OBJ (.obj) - ArchiCAD, Revit, SketchUp, Blender</option>
      <option value="dxf">DXF 3D faces (.dxf) - AutoCAD native</option>
      <option value="stl">STL (.stl) - any mesh tool</option>
    </select>
  ...
  function _init3D(W,H) {
    ...
    _scene.add(new THREE.HemisphereLight(0xbdd7ff,0x404452,0.55));
  ...
  function _sel(mesh){
    ...
    if(mesh.material&&mesh.material.emissive)mesh.material.emissive.setHex(0x007a66);
    _showInspector(mesh);
  }
  function _desel(){
  ...
  // ── Model export (OBJ / DXF / STL) ─────────────────────────────────────────
  // Writes the loaded geometry straight out of the viewer's meshes so the model opens in
  // AutoCAD (DXF), ArchiCAD / Revit / SketchUp / Blender (OBJ), or any mesh tool (STL).
  // Runs entirely in the browser - the model never leaves the machine.
  const _MAX_EXPORT_TRIS = 500000;

  function _worldTriangles() {
    const out = [];
    ...
    _meshMap.forEach(m => {
      ...
      let tri = [];
      for (let i = 0; i < n; i++) {
        v.fromBufferAttribute(pos, idx ? idx[i] : i).applyMatrix4(m.matrixWorld);
  ...
  function _f(n) { return (Math.round(n * 1e4) / 1e4).toString(); }
  ...
  function exportModel(fmt) {
    if (!fmt) return;
    const tris = _worldTriangles();
    if (!tris.length) {
      alert('No geometry to export yet. Open an IFC file and press "Load 3D" for the real geometry, then export.');
      return;
    }
    if (tris.length > _MAX_EXPORT_TRIS &&
        !confirm('This model has ' + tris.length.toLocaleString() + ' triangles. Writing that much may take a while or run out of memory. Export anyway?')) return;

    const base = 'VERIFIQ_model_' + new Date().toISOString().slice(0, 10);
    if (fmt === 'obj')      _download(_toObj(tris), base + '.obj', 'text/plain');
    else if (fmt === 'stl') _download(_toStl(tris), base + '.stl', 'application/sla');
    else if (fmt === 'dxf') _download(_toDxf(tris), base + '.dxf', 'application/dxf');
  }

  function _toObj(tris) {
    const L = ['# VERIFIQ - model export', '# Geometry from the loaded IFC model', 'o VERIFIQ_Model'];
    let i = 1;
    for (const t of tris) {
      for (let k = 0; k < 3; k++) L.push('v ' + _f(t[k][0]) + ' ' + _f(t[k][1]) + ' ' + _f(t[k][2]));
      L.push('f ' + i + ' ' + (i + 1) + ' ' + (i + 2));
      i += 3;
    }
    return L.join('\n');
  ...
  function _toStl(tris) {
    const L = ['solid VERIFIQ_Model'];
    for (const t of tris) {
      const n = _normal(t);
      L.push(' facet normal ' + _f(n[0]) + ' ' + _f(n[1]) + ' ' + _f(n[2]));
      L.push('  outer loop');
      for (let k = 0; k < 3; k++) L.push('   vertex ' + _f(t[k][0]) + ' ' + _f(t[k][1]) + ' ' + _f(t[k][2]));
      L.push('  endloop');
      L.push(' endfacet');
    }
    L.push('endsolid VERIFIQ_Model');
    return L.join('\n');
  }

  // Minimal DXF ENTITIES section of 3DFACE records - read natively by AutoCAD, and
  // importable by ArchiCAD / Revit through their DWG-DXF link.
  function _toDxf(tris) {
    const L = ['0','SECTION','2','ENTITIES'];
    for (const t of tris) {
      L.push('0','3DFACE','8','VERIFIQ');
      const pts = [t[0], t[1], t[2], t[2]];   // 4th corner repeats the 3rd for a triangle
      for (let k = 0; k < 4; k++) {
        L.push(String(10 + k), _f(pts[k][0]));
        L.push(String(20 + k), _f(pts[k][1]));
        L.push(String(30 + k), _f(pts[k][2]));
      }
    }
  ...
  return {
    ...
    toggleSev,
    selectAll,selectByGuid,zoomToSelected,goToFindings,exportModel,
