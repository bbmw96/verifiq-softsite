...
const DashboardPage = (() => {
  ...
  function renderWelcome(state) {
    ...
    return `
      ...
      <div class="card">
        ...
        <div class="three-col">
          ${capCard('20 IFC Data Levels + 206 Codes','20 IFC data levels (entity class → classification → SGPset_ → property values → georeferencing → geometry) PLUS 206 embedded IFC+SG classification codes with exact SGPset_ requirements per code per agency','var(--teal)')}
  ...
  function renderHealthBanner(session, dc) {
    ...
    return `<div class="card" style="margin-bottom:16px;background:linear-gradient(135deg,var(--navy-dark),var(--navy));color:white;border:none">
      <div style="display:flex;align-items:center;gap:24px;flex-wrap:wrap">
        ${scoreGauge(overall, grade, gradeCol, label)}
  ...
  // Radial donut compliance gauge - grade + percent in the centre, arc fills with the score.
  function scoreGauge(pct, grade, colour, label) {
    const p = Math.max(0, Math.min(100, pct));
    const R = 42, C = 2 * Math.PI * R, off = C * (1 - p / 100);
    return `<div style="text-align:center;flex-shrink:0">
      <svg width="128" height="128" viewBox="0 0 100 100" style="display:block">
        <circle cx="50" cy="50" r="${R}" fill="none" stroke="rgba(255,255,255,.12)" stroke-width="9"></circle>
        <circle cx="50" cy="50" r="${R}" fill="none" stroke="${colour}" stroke-width="9" stroke-linecap="round"
          stroke-dasharray="${C.toFixed(1)}" stroke-dashoffset="${off.toFixed(1)}"
          transform="rotate(-90 50 50)" style="transition:stroke-dashoffset .8s ease"></circle>
        <text x="50" y="47" text-anchor="middle" font-size="30" font-weight="900" fill="${colour}">${grade}</text>
        <text x="50" y="64" text-anchor="middle" font-size="11" font-weight="700" fill="#CBD5E1">${p.toFixed(0)}%</text>
      </svg>
